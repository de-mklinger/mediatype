...
class LinkedCaseInsensitiveMap<V> implements Map<String, V>, Serializable, Cloneable {
	 ...
	 * @see #convertKey(String)
	 */
	public LinkedCaseInsensitiveMap(@Nullable final Locale locale) {
		this(16, locale);
	}
	 ...
	 * @see #convertKey(String)
	 */
	public LinkedCaseInsensitiveMap(final int initialCapacity, @Nullable final Locale locale) {
		this.targetMap = new LinkedHashMap<String, V>(initialCapacity) {
			...
			protected boolean removeEldestEntry(final Map.Entry<String, V> eldest) {
				final boolean doRemove = LinkedCaseInsensitiveMap.this.removeEldestEntry(eldest);
				if (doRemove) {
	...
	private LinkedCaseInsensitiveMap(final LinkedCaseInsensitiveMap<V> other) {
	...
	public V get(final Object key) {
		if (key instanceof String) {
			final String caseInsensitiveKey = this.caseInsensitiveKeys.get(convertKey((String) key));
	...
	public V put(final String key, @Nullable final V value) {
		final String oldKey = this.caseInsensitiveKeys.put(convertKey(key), key);
		V oldKeyValue = null;
		if (oldKey != null && !oldKey.equals(key)) {
			oldKeyValue = this.targetMap.remove(oldKey);
		}
		final V oldValue = this.targetMap.put(key, value);
	...
	@Override
	@Nullable
	public V putIfAbsent(final String key, @Nullable final V value) {
		final String oldKey = this.caseInsensitiveKeys.putIfAbsent(convertKey(key), key);
		if (oldKey != null) {
	...
	public V remove(final Object key) {
		if (key instanceof String) {
			final String caseInsensitiveKey = this.caseInsensitiveKeys.remove(convertKey((String) key));
			if (caseInsensitiveKey != null) {
